	...
	def process_upload(self, video_id: int, chunk_index: int, chunk: bytes):
		...
		with transaction.atomic(): #statement
			...
			if video_media.status == VideoMedia.Status.PROCESS_STARTED:
				raise VideoMediaInvalidStatusException('An upload is already in progess.')

			#Se o status já esta em 'UPLOADED_STARTED', continua arnazenando os chunks
			...
			# Se o processamento já terminou, reinicia o caminho do diretorio de chunks
			if video_media.status == VideoMedia.Status.PROCESS_FINISHED:
				video_media_path = directory
				video_is_published = False
				video.save()
